...
namespace SwizlyPeasy.Gateway.Mediator.handler
{
    public class LogoutHandler : IRequestHandler<LogoutRequest, Unit>
    {
        ...
        public async Task<Unit> Handle(LogoutRequest request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Logout for ip {RemoteIpAddress}.", _contextAccessor.HttpContext.Connection.RemoteIpAddress);

            var siteCookies =
                _contextAccessor.HttpContext.Request.Cookies.Where(
                    c => c.Key.StartsWith(Common.Constants.Cookie));
            foreach (var cookie in siteCookies)
            {
                _contextAccessor.HttpContext.Response.Cookies.Delete(cookie.Key);
            }

            await _contextAccessor.HttpContext.SignOutAsync(Common.Constants.CookiesAuthenticationProviderKey);
            ...
            _logger.LogWarning("Token Endpoint: {tokenEndpoint}, Revocation Endpoint: {revocationEndpoint}.", tokenEndpoint, revocationEndpoint);
